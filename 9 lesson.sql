...
FROM Customers C
JOIN Sales S  ON C.CustomerID=S.ProductID
...
FROM Courses C
JOIN Enrollments E ON C.CourseID=E.EnrollmentID
...
SELECT D.DepartmentName,E.Name
FROM Employees E
LEFT JOIN  Departments D ON   D.DepartmentID=E.EmployeeID
...
select p.ProductName,o.Quantity,pp.Amount
from Products as p
join Payments as pp on pp.PaymentID=p.ProductID
join Orders as o on p.ProductID=o.ProductID
WHERE pp.Amount <> (o.Quantity * pp.Amount);
SELECT 
    p.ProductName,
   o.Quantity,
    pp.Amount 
FROM Payments AS pp
JOIN Orders AS o ON pp.OrderID = o.OrderID
...
select e.Name,e.Salary,m.ManagerID
from Employees e
join Employees m on m.ManagerID=e.EmployeeID
where m.Salary>=m.Salary
