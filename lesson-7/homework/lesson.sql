...
select min (Price) as sum_price from Products 
...
select count (*) as CustomerID_rows from Customers  
--4-Write a query to count the number of unique product categories from the Products table. 

select* from Products 

select count ( distinct category) as category_spr from Products 
...
--5-Write a query to calculate the average age of employees in the Employees table. 
select * from Employees 
select avg (Age) as Age_avg from Employees 

--6-Write a query to count the number of employees in each department. 
select * from Employees 
select DepartmentName, count (EmployeeID) as EmployeeID_soni from Employees 
group by DepartmentName 


--7-Write a query to show the minimum and maximum Price of products grouped by Category. Use products table. 
select * from Products  
...
--9-Write a query to filter departments having more than 5 employees from the Employees table.(DeptID is enough, if you don't have DeptName).
...
select * from Sales 
select ProductID, sum(saleamount) as cnt_sale,
avg(saleamount) as cnt_avg from Sales
group by ProductID  

12. Write a query to count the number of employees from the Department HR. 
select * from employees 

select DepartmentName, count(EmployeeID) as cnt_DepartmentName 
from employees 
group by DepartmentName 
...
select DepartmentName, count (*) as count_dept, avg(Salary) as avg_salary
from Employees 
group by DepartmentName
...
select Category, avg(price) as avg_category
from Products
group by Category 
...
select SaleDate, sum(SaleAmount) as total_year
from Sales 
group by SaleDate 
...
select CustomerID, sum(SaleAmount)as sum_sale
from Sales 
group by CustomerID
having sum(SaleAmount)>3


19. Write a queryWrite a query that shows the average price for each product category, and then filter categories with an average price greater than 150. 
select * from Products 

select Category, avg(price) as total_avg
from Products
group by Category
...
select CustomerID, sum(SaleAmount) as cnt_sum
from Sales 
group by CustomerID 
...
select EmployeeID, DepartmentName, sum (Salary) as sum_total, avg(Salary) as avg_total
from Employees 
...
select CustomerID, sum(case when TotalAmount>50 then TotalAmount else 0 end) as sum_amount, min(TotalAmount) as min_amount
from Orders 
group by CustomerID 
...
select ProductID, OrderDate, sum(TotalAmount) as sum_total,
...
select OrderDate, min(Quantity) as min_quantity, max(Quantity) as max_quantity 
from Orders
group by OrderDate 


--26. Write a query to find the MIN and MAX order quantity per Year. From orders table. to filter out Departments with average salary expenses greater than 60000.(DeptID is enough, if you don't have DeptName).   
select *from Employees
