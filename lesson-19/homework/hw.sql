...
BEGIN
    ...
    -- Step 2: Insert data with bonus calculation
    INSERT INTO #EmployeeBonus (EmployeeID, FullName, Department, Salary, BonusAmount)
    SELECT 
        e.EmployeeID,
        CONCAT(e.FirstName, ' ', e.LastNam
...
AS
BEGIN
    -- Update salaries by increasing with the given percentage
    UPDATE Employees
...
SELECT 
    ...
    CASE
        ...
        WHEN c.ChildCount = 0 THEN 'Leaf'
        ELSE 'Inner'
    END AS type
FROM 
    Tree t
LEFT JOIN 
    (SELECT p_id, COUNT(*) AS ChildCount
     FROM Tree
     WHERE p_id IS NOT NULL
     GROUP BY p_id) c ON t.id = c.p_id
ORDER BY t.id;
...
SELECT 
    s.user_id,
    COALESCE(
        CAST(SUM(CASE WHEN c.action = 'confirmed' THEN 1 ELSE 0 END) AS FLOAT) / NULLIF(COUNT(c.action), 0),
        0
...
BEGIN
    SELECT
        ...
        CASE 
            WHEN SUM(s.Quantity) IS NULL THEN NULL
