...
WITH RecursiveFamily AS (
    ...
    -- Recursive step: find descendants of children
...
```
--- 
-- Update fullname if it's NULL or empty
UPDATE #residents
SET fullname = 
    SUBSTRING(
        address,
...
WITH RecursiveRoutes AS (
    -- Anchor member: start at Tashkent
    SELECT 
    ...
    -- Recursive member: join routes where ArrivalCity = next DepartureCity, avoid cycles
    SELECT 
        CAST(rr.Route + ' - ' + r.ArrivalCity AS VARCHAR(MAX)) AS Route,
        r.ArrivalCity,
        rr.Cost + r.Cost,
        r.RouteID
...
-- Select only routes that reach Khorezm
, CompleteRoutes AS (
    SELECT Route, Cost
...
-- Select cheapest and most expensive routes
SELECT Route, Cost FROM CompleteRoutes
WHERE Cost = (SELECT MIN(Cost) FROM CompleteRoutes)
   OR Cost = (SELECT MAX(Cost) FROM CompleteRoutes)
ORDER BY Cost;
...
WITH Numbered AS (
    SELECT
        Id,
        Vals,
        ROW_NUMBER() OVER (PARTITION BY Id ORDER BY (SELECT NULL)) AS rn -- assign row number per Id
    FROM #Consecutives
),
LastValue AS (
    SELECT
        ...
        Vals AS LastVal
    FROM Numbered
    WHERE rn = (SELECT MAX(rn) FROM Numbered N2 WHERE N2.Id = Numbered.Id)
),
ConsecutiveGroups AS (
    SELECT
        ...
        Vals,
        rn,
        rn - ROW_NUMBER() OVER (PARTITION BY Id, Vals ORDER BY rn) AS grp -- group consecutive same values
    FROM Numbered
),
MaxConsecutive AS (
    SELECT
...
SELECT
    L.Id,
    CASE WHEN L.LastVal = 1 THEN 0 ELSE 1 END AS NextVal,
    M.MaxConsecutive
FROM LastValue L
JOIN MaxConsecutivePerId M ON L.Id = M.Id;
SELECT 
    E.EmployeeID,
    E.EmployeeName,
    E.Department,
...
JOIN
    (
        ...
        FROM #EmployeeSales
        GROUP BY Department
    ) DeptAvg ON E.Department = DeptAvg.Department
WHERE
    E.SalesAmount > DeptAvg.AvgSales
ORDER BY
    E.Department, E.EmployeeName;
...
```
---
SELECT DISTINCT E.EmployeeID, E.EmployeeName, E.Department, E.SalesAmount, E.SalesMonth, E.SalesYear
...
WHERE NOT EXISTS (
    ...
    WHERE NOT EXISTS (
        ...
        WHERE E2.EmployeeID = E.EmployeeID
          AND E2.SalesMonth = S.SalesMonth
          AND E2.SalesYear = S.SalesYear
    )
)
