...
---
-- Step 1: Get the max Vals per ID
WITH MaxVals AS (
    SELECT 
        ID, 
        MAX(Vals) AS MaxVal
...
-- Step 2: Join back to get corresponding Item(s)
SELECT 
    T.ID,
    T.Item,
    T.Vals
...
| 102 |    6    |  
```
-- Step 1: Get max Vals for each Id and DetailedNumber
WITH MaxVals AS (
    SELECT 
        Id,
        DetailedNumber,
...
SELECT 
    ...
    ROUND(100.0 * SUM(QuantitySold * UnitPrice) / (SELECT SUM(QuantitySold * UnitPrice) FROM Sales), 2) AS PercentageContribution
FROM Sales
GROUP BY Category;
